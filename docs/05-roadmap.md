...
### วิดีโอในหน้าร้าน (แทรกหลัง 1c)
- [x] อัปคลิปสั้นตรงได้ (MP4/WebM/MOV ≤ 60 วิ ≤ 40 MB) — **ไม่ transcode**
      ดึงเฟรมมาทำภาพปกในเบราว์เซอร์ กริดจึงไม่โหลดวิดีโอจนกว่าจะกดเล่น
- [x] ฝังลิงก์ YouTube/Vimeo — เก็บเป็น `provider:id` ไม่ใช่ URL ทั้งก้อน
      whitelist โดเมน + ประกอบ URL ใหม่ตอนแสดง (ค่านี้จบลงใน `src` ของ iframe)
      ใช้ youtube-nocookie และไม่โหลด iframe จนกว่าผู้ใช้จะกด
- [x] ขยับโควตาฟรี: 300 MB → **2 GB** · ต่อไฟล์ 20 → **50 MB** · ผลงาน 12 → **30 ชิ้น**
      คิดจากต้นทุนจริงแล้ว egress คือค่าใช้จ่าย ไม่ใช่พื้นที่เก็บ (ดู lib/billing/plans.ts)
...
- [x] อีเมลผ่าน Resend — ส่งหลัง response ด้วย `after()` ไม่บล็อกการกดปุ่ม
      ส่งเฉพาะเหตุการณ์ที่พลาดแล้วเสียงาน: ออเดอร์ใหม่ · แจ้งโอน · ยืนยันเงินเข้า
      (ข้อความในเธรดถี่เกินไป — รอระบบ digest ตามแผน docs/01 §6)
      ⚠️ **ถึงลูกค้าจริงยังไม่ได้** จนกว่าจะยืนยันโดเมนใน Resend ซึ่งต้องมีโดเมนก่อน
      ตอนนี้ส่งได้เฉพาะอีเมลเจ้าของบัญชี Resend — ทดสอบผ่านแล้วทั้งไทยและอังกฤษ

**เพิ่มหลัง research คู่แข่ง (docs/00 §3) — เรียงตามความสำคัญ**

- [x] 🔴 **คำเตือนก่อนแสดง QR ว่าโอนแล้วเรียกคืนไม่ได้** และเปลี่ยนคำปุ่มยืนยันจาก
...
- [x] ✅ **ครีเอเตอร์ต้องตอบรับก่อนลูกค้าจ่ายได้** (`canPay` + ด่านใน `recordPayment`)
- [ ] 🟠 **ใบเสนอราคาที่ครีเอเตอร์ตั้งเอง** — แผนเต็มอยู่ที่ `docs/06-quotes-and-invites.md`
- [ ] 🟠 **เชิญลูกค้าที่ยังไม่มีบัญชี** — แบบเดียวกัน ยังไม่ผ่านการหาช่องโหว่ ห้ามสร้างก่อนตรวจ
